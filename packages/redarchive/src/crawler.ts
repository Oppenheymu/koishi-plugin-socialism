...
export class Crawler {
    ...
    async listCategories(): Promise<CategoryItem[]> {
        ...
        const cached = await readCache<CategoryItem[]>(
        ...
        const { html, usedUrl } = await tryFetchEntry(
            this.config.entryUrls,
        ...
        if (categories.length) {
            await writeCache(
                ...
                cacheKey,
                categories,
                this.config.categoryCacheTtlMs,
            );
        }
        return categories;
    }
    ...
    async listDocuments(categoryUrl: string): Promise<DocumentItem[]> {
        const cached = await readCache<DocumentItem[]>(
            this.config.cacheDir,
            "documents",
            categoryUrl,
        );
        ...
        if (documents.length) {
            await writeCache(
                ...
                categoryUrl,
                documents,
                this.config.documentCacheTtlMs,
            );
        }
        return documents;
    }

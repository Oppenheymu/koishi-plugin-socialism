...
/**
 * 计算音频缓存目录：使用 koishi 的 baseDir/data/redseries/redmusic。
 * 红色系列插件统一聚合在 data/redseries/ 下，koishi 在 ctx 上提供 baseDir（实例根目录）。
 */
export function getCacheDir(ctx: Context): string {
    return resolve(ctx.baseDir, "data", "redseries", "redmusic");
}
...
export async function downloadAssets(
    ...
    try {
        ...
        const missing = catalog.filter((e) => !existsSync(resolve(cacheDir, e.file)));
        if (missing.length) {
            logger.error("解压后仍有 %d 个文件缺失，首例: %s", missing.length, missing[0]?.file);
            return false;
        }

...
export function filterSongs(catalog: SongEntry[], filter?: SongFilter): SongEntry[] {
...
export function randomPick(probability: number, filter?: SongFilter): RandomOutcome {
...
export function exactPick(filter: SongFilter): { entry: SongEntry; audio: string } | null {
    const catalog = loadCatalog();
    const pool = filterSongs(catalog, filter);
